...
async def _upgrade_guest_account(db: Database, mail: Mail, user: User, user_create: UserCreate) -> None:
    ...
    # Flush to stage the changes within the transaction without
    # committing — this lets us rollback if the email fails.
    await db.flush()
    ...
    try:
        ...
        if cfg.DEBUG and cfg.DEBUG_ALLOW_REGISTRATION_WITHOUT_EMAIL:
            register_logger.warning(
                "Email delivery failed but"
                " DEBUG_ALLOW_REGISTRATION_WITHOUT_EMAIL"
                " is set. Guest account '%s' upgraded"
...
async def _register_anonymous_user(
    ...
    if not share_link.allow_anonymous_access:
        raise AppException(
            status_code=403,
            detail="Anonymous access is not allowed with this sharelink",
