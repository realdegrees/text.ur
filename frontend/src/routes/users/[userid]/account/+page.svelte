...
<div class="flex h-full w-full flex-col items-center justify-start overflow-y-auto">
	<div class="flex h-full w-[30%] flex-col items-start justify-start gap-6 p-6">
		...
		{#if data.sessionUser.is_guest && showUpgradeForm}
			<div class="w-full rounded-lg bg-inset p-6 shadow-inner shadow-black/50">
				...
				>
					...
					<button type="submit" disabled={isUpgrading} class="w-full btn-primary py-3">
						{#if isUpgrading}
		...
		>
			...
			<button type="submit" disabled={isUpdating} class="w-full btn-primary py-3">
				{#if isUpdating}
					<Loading class="m-auto" />
				{:else}
		...
		{#if !data.sessionUser.is_guest}
			<DangerZone
				title={$LL.userSettings.danger.title()}
				description={$LL.userSettings.danger.description()}
				actionLabel={$LL.userSettings.danger.deleteButton()}
				confirmText={data.sessionUser.username}
				confirmTitle={$LL.userSettings.danger.confirmTitle()}
				confirmPrompt={$LL.userSettings.danger.confirmMessage({ name: data.sessionUser.username })}
