<script lang="ts">
	...
	async function handleSave(): Promise<void> {
		...
		if (result.success) {
			...
			await invalidate('app:route-membership');
				...
				<div class="flex flex-col items-start gap-6 lg:flex-row">
					...
					<div class="flex w-full flex-col gap-3 lg:w-auto lg:min-w-64">
						<h3 class="text-sm font-semibold text-text/80">
							{$LL.groupSettings.scoring.actionPoints()}
						</h3>
						...
						<div class="flex flex-row justify-end">
							...
							>
								...
								<span
									>{scoreSaving
										? $LL.groupSettings.scoring.savingPoints()
										: $LL.groupSettings.scoring.savePoints()}</span
								>
							</button>
						</div>
					</div>
					...
					<div class="flex w-full flex-col gap-3 lg:flex-1">
						<h3 class="text-sm font-semibold text-text/80">
							{$LL.groupSettings.scoring.emojiReactions()}
						</h3>
