	...
	<div class="flex flex-col gap-4">
		...
		{#if legendOpen}
			<div class="rounded-lg border border-text/10 bg-text/[0.02] p-3 text-xs text-text/60">
				<p class="mb-3 font-medium text-text/70">How points are earned</p>
						...
						<table class="w-full">
							<thead>
								<tr class="border-b border-text/10 text-left text-text/50">
									...
									<th class="pb-1.5 text-right font-medium">Points</th>
								</tr>
							</thead>
							<tbody>
								<tr class="border-b border-text/5">
									<td class="py-1.5">Create a highlight</td>
									<td class="py-1.5 text-right font-medium text-text/80"
										>{scoreConfig?.highlight_points ?? 1}</td
									>
								</tr>
								<tr class="border-b border-text/5">
									<td class="py-1.5">Write a comment</td>
									<td class="py-1.5 text-right font-medium text-text/80"
										>{scoreConfig?.comment_points ?? 5}</td
									>
								</tr>
								<tr>
									<td class="py-1.5">Add a tag</td>
									<td class="py-1.5 text-right font-medium text-text/80"
					...
					{#if legendReactions.length > 0}
						<div class="w-full sm:flex-1">
							<table class="w-full">
								<thead>
									<tr class="border-b border-text/10 text-left text-text/50">
										<th class="pb-1.5 font-medium">Emoji</th>
										<th class="pb-1.5 text-right font-medium">Received</th>
										<th class="pb-1.5 text-right font-medium">From admin</th>
										<th class="pb-1.5 text-right font-medium">Giver</th>
									</tr>
								</thead>
								<tbody>
		...
		{#if reactionBreakdown.length > 0}
			<h3 class="mt-2 text-sm font-semibold text-text/70">Reactions</h3>
			<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3" class:opacity-50={loading}>
						...
						<div class="mt-2 flex items-baseline justify-between">
							<div class="flex flex-col">
								...
								<div class="flex gap-3 text-[11px] text-text/40">
									<span>{rb.received_count} received</span>
									<span>{rb.given_count} given</span>
								</div>
							</div>
							<span class="text-sm text-text/40">{totalPoints} pts</span>
						</div>
					</div>
				{/each}
			</div>
