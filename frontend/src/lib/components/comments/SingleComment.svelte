...
{#if expanded}
	...
	<div
		...
		class="absolute right-4 left-0 grid gap-2 rounded border-l-4 bg-white px-3 py-2 shadow-md transition-all duration-200"
		...
		{:else}
			...
			<div class="grid gap-2">
				<div class="flex items-center justify-between gap-2">
					<span class="min-w-0 text-xs font-medium wrap-break-word text-gray-600">
				...
				{#if comment.content}
					<p class="text-sm leading-snug wrap-break-word text-gray-800">{comment.content}</p>
				{:else}
...
{:else}
	...
	<div
		data-comment-id={comment.id}
		class="absolute right-4 max-w-full cursor-pointer rounded-full px-2 py-1 shadow-sm transition-all duration-200 hover:shadow-md"
		style:top="{actualTop}px"
		...
		tabindex="0"
	>
		<span class="text-xs font-medium wrap-break-word text-gray-800">
			{comment.user?.username ?? 'Anonymous'}
		</span>
	</div>
{/if}
